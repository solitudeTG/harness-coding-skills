...
applies_to: [harness-skills, gate-design, hook-design, harness-delegation-gate, harness-start-gate, harness-readiness-dashboard, session-recovery]
created: 2026-05-30
updated: 2026-05-30
---
...
Delegation Gate 原先把 `not needed`、`ask user`、`authorized`、`declined`、`required`、`blocked`、`conditional` 混在同一层 outcome 中。这里面有些是主 Agent 的下一步行动，有些是权限来源，有些是风险说明，有些是收尾状态。

结果是 Agent 很难稳定回答最关键的问题：我现在应该自己做、委托做，还是停下。类似问题还可能出现在 gate、hook、readiness、closeout 等机制里：outcome 超过 3-4 个，多个值不改变下一步行动，或缺失信息被伪装成业务 outcome。
